        ...
        1: [function (require, module, exports) {
            ...
            function barChartPlotter(e) {
                ...
                for (var i = 0; i < points.length; i++) {
                    ...
                    ctx.fillRect(center_x - bar_width / 4, p.canvasy, bar_width, y_bottom - p.canvasy);
                    ctx.strokeRect(center_x - bar_width / 4, p.canvasy, bar_width, y_bottom - p.canvasy);
                }
            }
            (function () {
                try {
 ...
 */Dygraph.prototype.__init__ = function (div, file, attrs) {
                    ...
                    for (var i = 0; i < this.plugins_.length; i++) {
                        ...
                        for (var eventName in plugin_dict.events) {
                            ...
                            var callback = plugin_dict.events[eventName]; var pair = [plugin_dict.plugin, callback];
                            if (!(eventName in this.eventListeners_)) { this.eventListeners_[eventName] = [pair]; }
                            else { this.eventListeners_[eventName].push(pair); }
                        }
 ...
 */Dygraph.prototype.addAndTrackEvent = function (elem, type, fn) { utils.addEvent(elem, type, fn); this.registeredEvents_.push({ elem: elem, type: type, fn: fn }); }; Dygraph.prototype.removeTrackedEvents_ = function () { if (this.registeredEvents_) { for (var idx = 0; idx < this.registeredEvents_.length; idx++) { var reg = this.registeredEvents_[idx]; utils.removeEvent(reg.elem, reg.type, reg.fn); } } this.registeredEvents_ = []; }; // Installed plugins, in order of precedence (most-general to most-specific).
                ...
                if (g.getOptionForAxis('drawAxis', 'x')) {
                    if (layout.xticks) {
                        ...
                        layout.xticks.forEach(function (tick) {
                            if (HourOrDailyFlag == "Daily")
                                if (tick.label === undefined || tick.label.indexOf("#") == -1) return; // this tick only has a grid line.
                            else 
            ...
            */
            ...
            var calculateEmWidthInDiv = function calculateEmWidthInDiv(div) {
                ...
                sizeSpan.setAttribute('style', 'margin-left: 60px; padding: 0 0 0 1em; border: 0;');
            ...
            Legend.defaultFormatter = function (data) {
                ...
                var html = '<div style="border: #D3D3D3;border-style: solid;"> <div style="margin-left:5px;">';//;
                ...
                if (typeof data.x === 'undefined') {
                    ...
                    for (var i = 0; i < data.series.length; i++) {
                        ...
                        html += " <span style='font-weight: bold; color: " + series.color + ";'>" + series.dashHTML + " " + series.labelHTML.split('_')[0] + "</span>";
                    }
                    return html;
                }

                if (data.series[0].labelHTML.includes('(HH:mm:ss)')) {
                    var firstDate = new Date(g.file_[0][0]);
                    var lastDate = new Date(g.file_[g.file_.length - 1][0]);
                    var currDate = new Date(data.xHTML);
                    if (currDate.getTime() == firstDate.getTime() || currDate.getTime() == lastDate.getTime())
                        return "";
                }

                var tooltop = (new Date(data.xHTML).getDate()) + '-' +
                    (new Date(data.xHTML).getMonth() == 0 ? "Jan" :
                        new Date(data.xHTML).getMonth() == 1 ? "Feb" :
                            new Date(data.xHTML).getMonth() == 2 ? "Mar" :
                                new Date(data.xHTML).getMonth() == 3 ? "Apr" :
                                    new Date(data.xHTML).getMonth() == 4 ? "May" :
                                        new Date(data.xHTML).getMonth() == 5 ? "Jun" :
                                            new Date(data.xHTML).getMonth() == 6 ? "Jul" :
                                                new Date(data.xHTML).getMonth() == 7 ? "Aug" :
                                                    new Date(data.xHTML).getMonth() == 8 ? "Sept" :
                                                        new Date(data.xHTML).getMonth() == 9 ? "Oct" :
                                                            new Date(data.xHTML).getMonth() == 10 ? "Nov" :
                                                                new Date(data.xHTML).getMonth() == 11 ? "Dec" : '')
                    + '-' + new Date(data.xHTML).getFullYear() + ' ';

                if (!data.series[0].labelHTML.includes('(HH:mm:ss)')) {
                    tooltop += (new Date(data.xHTML).getHours() < 10 ? '0' + new Date(data.xHTML).getHours() : new Date(data.xHTML).getHours()) + ':'
                        + (new Date(data.xHTML).getMinutes() < 10 ? '0' + new Date(data.xHTML).getMinutes() : new Date(data.xHTML).getMinutes()) + ':'
                        + (new Date(data.xHTML).getSeconds() < 10 ? '0' + new Date(data.xHTML).getSeconds() : new Date(data.xHTML).getSeconds());
                }

                html += "<span> <b> " + tooltop + " </b> </span> </Br> <hr style='width:100%;text-align:left;margin-left:0;margin-top:3px;margin-Bottom:3px;'>";

                // debugger;
                for (var i = 0; i < data.series.length; i++) {

                    var series = data.series[i];
                    var seriesName = series.labelHTML.split('_')[0];
                    var unit = "";

                    if (seriesName == 'VR' || seriesName == 'VY' || seriesName == 'VB')
                        unit = 'V';
                    else if (seriesName == 'IR' || seriesName == 'IY' || seriesName == 'IB')
                        unit = 'A';
                    else if (seriesName == 'Active Energy')
                        unit = 'kWh';
                    else if (seriesName == 'Apparent Energy')
                        unit = 'kVAh';
                    else if (seriesName == 'Reactive Energy Lag')
                        unit = 'kVArh';
                    else if (seriesName == 'Reactive Energy Lead')
                        unit = 'kVArh';
                    else if (seriesName == 'Active Power')
                        unit = 'W';
                    ...
                    if (seriesName.includes('(HH:mm:ss)')) {
                        if (series.yHTML != "0") {
                            value = (parseInt(series.yHTML) == 0 ? "00" : (parseInt(series.yHTML) < 10 ? ("0" + parseInt(series.yHTML)) : parseInt(series.yHTML)))
                                + ':' +
                                (parseInt((series.yHTML - parseInt(series.yHTML)) * 60) == 0 ? "00" : (parseInt((series.yHTML - parseInt(series.yHTML)) * 60) < 10 ? ("0" + parseInt((series.yHTML - parseInt(series.yHTML)) * 60)) : (parseInt((series.yHTML - parseInt(series.yHTML)) * 60))));
                        }
                        else {
                            value = "00:00:00";
                        }
                    ...
                    html += "<span " + cls + " style='color: " + series.color + ";'><b>" + seriesName + " : " + value + " " + unit + "</b> </span> </Br>";
                }
                html += "</div> </div>";
                return html;
            };
            ...
            function generateLegendDashHTML(strokePattern, color, oneEmWidth) {
                ...
                for (j = 0; j < loop; j++) {
                    for (i = 0; i < segmentLoop; i += 2) {
                        ...
                        //
                        dash += "<div class=\"dygraph-legend-dash\" style=\"margin-right: " + marginRight + "em; padding-left: " + paddingLeft + 100 + "em;\"></div>";
                    }
                }
                return dash;
            };
